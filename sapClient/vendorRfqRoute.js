...
const ENTITY_SET = 'ZRFQ_VENDORSet'; // Your RFQ entity set
const SAP_CREDS = {
  username: 'K901577',
  password: 'Haris@0713',
};
...
router.get('/:vendorId', async (req, res) => {
  ...
  console.log('📥 Fetching RFQ for Vendor ID:', vendorId);
  ...
  try {
    ...
    if (!results || results.length === 0) {
      return res.status(404).json({ success: false, message: 'No RFQ data found for the vendor.' });
    }

    // Transform RFQ data
    const rfqList = results.map(item => ({
      Lifnr: item.Lifnr,
      Ebeln: item.Ebeln,
      Aedat: item.Aedat,
      Bedat: item.Bedat,
    ...
    console.error('❌ SAP RFQ Fetch Error:', {
    ...
    return res.status(error.response?.status || 500).json({
      success: false,
      message: 'Failed to fetch vendor RFQ data',
      error: error.message,
